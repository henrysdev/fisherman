defmodule FishermanServerWeb.Live.LinearShellsTable do
  @moduledoc """
  Entry point for linear shell feed liveview. Mounts a liveview that
  registers as a subscriber to the user whos history is being
  recorded. Displays a timestamp-based linear view of commands
  ...
  def render(assigns) do
    ...
    <%= live_component @socket,
        FishermanServerWeb.Live.LinearShellsTable.LinearShellsTableComponent,
  ...
  def refresh_feed_state(first_ts, _curr_dt, user_id) do
    ...
    %{
      records: Query.shell_records_since_dt(first_dt, user_uuid),
      row_info: %{
        latest_ts: latest_ts,
        first_ts: first_ts,
